...
import { roles_sistema, planes, organizaciones } from "@/lib/schema";
...
export default async function PermisosMaintenancePage() {
  ...
  // Obtener organización actual para saber su plan
  const organizacion = await db.query.organizaciones.findFirst({
    where: eq(organizaciones.id, usuario.organizacion_id)
  });

  // Obtener roles de la organización actual y estructura del sistema filtrada por plan
  const [roles, planesDisponibles, estructura] = await Promise.all([
    ...
    getEstructuraPermisos(organizacion?.plan_id || undefined)
  ]);
