...
import { secciones_contenido, parametros_landing, testimonios, preguntas_frecuentes, telefonos_emergencia, responsables_organizacion, galeria_fotos } from "@/lib/schema";
...
export default async function ContenidoAdminPage() {
  ...
  const [secciones, parametros, testimoniosData, faq, telefonos, responsables, galeria] = await Promise.all([
    ...
    db.select().from(galeria_fotos).where(eq(galeria_fotos.organizacion_id, orgId)).orderBy(desc(galeria_fotos.orden)),
  ]);
