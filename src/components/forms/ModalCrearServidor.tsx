"use client"

import * as React from "react"
import { Plus } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog"
import NuevoServidorForm from "./NuevoServidorForm"

interface ModalCrearServidorProps {
  sedes: any[]
}

export function ModalCrearServidor({ sedes }: ModalCrearServidorProps) {
  const [open, setOpen] = React.useState(false)

  return (
    <>
      <button 
        onClick={() => setOpen(true)}
        className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 dark:bg-[#e11d48] dark:hover:bg-[#be123c] text-white px-4 py-2 rounded-md text-sm font-medium transition-colors shadow-sm"
      >
        <Plus className="w-4 h-4" />
        Añadir Servidor
      </button>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent 
          className="max-w-4xl max-h-[90vh] overflow-y-auto dark:bg-[#1a1b26] dark:border-[#2a2b3d] p-0"
          onPointerDownOutside={(e) => e.preventDefault()} // REGLA DE ORO: No cerrar al clic afuera
        >
          <DialogHeader className="p-6 border-b border-slate-100 dark:border-[#3b3c54]">
            <DialogTitle className="text-xl font-bold dark:text-white">Nuevo Registro de Servidor</DialogTitle>
          </DialogHeader>
          
          <div className="p-6">
            {/* Reusamos el componente de formulario pero le pasamos un callback para cerrar al terminar */}
            <NuevoServidorForm 
              sedes={sedes} 
              onSuccess={() => setOpen(false)} 
              isModal={true}
            />
          </div>
        </DialogContent>
      </Dialog>
    </>
  )
}
